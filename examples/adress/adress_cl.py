...
Experiment of using ADRess in synthetic clarinet chords.
...
left_intensities = np.array([0.75, 0.50, 0.25, 0.55])
note_duration = 1
samp_rate = 44100
...
left_sep_names = ["Tenor (Melody)", "Alto (Chord formation)"]
right_sep_names = ["Bass (Chord formation)", "Soprano (Melody)"]
true_names = ["soprano", "alto", "tenor", "bass"]   # True part names
name = f"melody-chord-cl={str(left_intensities).replace(' ', ',')}_sr={samp_rate}"
...
for i in range(melody_data.shape[0]):
    wav.write(f"audio/{name}_true_{true_names[i]}", melody_data[i], samp_rate=samp_rate)
